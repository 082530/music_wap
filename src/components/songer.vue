<template>
    <div class="songer">
      <div>
        <img :src="`${artist.picUrl}?param=400y400`" alt="">
        <p>
      ...
      <div class="list">
        <p>
          <span @click="type=0" :class="type==0?'active':''">热门单曲</span>
          <span @click="type=1" :class="type==1?'active':''">专辑<b>{{artist.albumSize}}</b></span>
          <span @click="type=2" :class="type==2?'active':''">视频<b>{{artist.mvSize}}</b></span>
          <span @click="type=3" :class="type==3?'active':''">艺人信息</span>
        </p>
        <!--热门单曲-->
        <table v-show="type==0">
          <tbody>
          <tr v-for="(item,key) in hotlist">
            <td>{{key+1}}</td>
            <td>
          ...
          <ul>
            <li v-for="item in albumlist">
              <router-link :to="{name:'album',params: {id:item.id}}">
                ...
                <div>
                  <p>{{item.name}}<b v-if="item.alias[0]">({{item.alias[0]}})</b></p>
                  <span>发行时间：{{new Date(item.publishTime).toLocaleDateString()}}</span>
                </div>
              </router-link>
            </li>
          </ul>
          <button class="morelist" v-if="morelist" @click="more1()">加载更多</button>
        </div>
        <!--视频-->
        <div v-show="type==2" class="album">
          <ul>
            <li v-for="item in mvlist">
              <router-link :to="{name:parseInt(item.id)==item.id?'play':'play1',params:{id:item.id}}">
                <img :src="`${item.imgurl}?param=100y80`" alt="">
                <div>
                  <p>{{item.name}}</p>
          ...
          <button class="morelist" v-if="mvbutton" @click="mv1()">加载更多</button>
        </div>
        <!--简介-->
        <div v-show="type==3" class="song">
          <p>{{artist.name}}简介</p>
          <div>
{{artist.briefDesc}}
          </div>
...
export default {
  ...
  methods: {
    ...
    more1 () {
      this.$.get(`${this.url}/artist/album?limit10&offset=${10 * this.page++}&id=${this.id}`).then(res => {
        this.albumlist = this.albumlist.concat(res.data.hotAlbums)
        res.data.more ? this.morelist = 1 : this.morelist = 0
      })
    },
    mv1 () {
      this.$.get(`${this.url}/artist/mv?limit10&offset=${10 * this.page1++}&id=${this.id}`).then(res => {
        this.mvlist = this.mvlist.concat(res.data.mvs)
        res.data.hasMore ? this.mvbutton = 1 : this.mvbutton = 0
      })
    }
  },
  beforeMount () {
    this.$.get(`${this.url}/artists?id=${this.id}`).then(res => {
      this.artist = res.data.artist
      this.hotlist = res.data.hotSongs
    })
    this.$.get(`${this.url}/artist/album?limit=10&id=${this.id}`).then(res => {
      this.albumlist = res.data.hotAlbums
      res.data.more ? this.morelist = 1 : this.morelist = 0
    })
    this.$.get(`${this.url}/artist/mv?offset=0&limit=10&id=${this.id}`).then(res => {
